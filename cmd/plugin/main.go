...
func run(stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if version == "" {
		_, _ = fmt.Fprintln(stderr, "updater-python: SEMREL_VERSION is required")
		return 1
	}
	...
	if getenv("SEMREL_DRY_RUN") == "true" {
		_, _ = fmt.Fprintf(stdout, "updater-python: [dry-run] would update %s to version %s\n", file, version)
		return 0
	}

	if err := plugin.NewUpdater().Update(file, version); err != nil {
		_, _ = fmt.Fprintln(stderr, "updater-python:", err)
		return 1
	}

	_, _ = fmt.Fprintf(stdout, "updater-python: updated %s to version %s\n", file, version)
	return 0
}
